...
var iframes = require('./components/iframes.js');
...
class Client {
  ...
  enhanced_pdp_modal(charges, type = 'pdp') {
    ...
    const url = platform.pdpHost(platform.marketingUI, this.platform) + '/pdp/' + this.key + '/' + type + '/' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.discount_amount, charges.data.grand_total].join(',');
    return modal(url);
  }
  get_apply_now(type, charges) {
    const url = platform.pdpHost(platform.marketingUI, this.platform) + '/apply.html?public_key=' + this.key + '&type=' + type + '&charges=' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.discount_amount, charges.data.grand_total].join(',');
    return iframes.frame(url);
  }
  get_checkout_display(charges) {
    if (charges && typeof charges !== 'object') {
      return reject('charges should be a charges object');
    }
    const url = platform.pdpHost(platform.marketingUI, this.platform) + '/checkout.html?public_key=' + this.key + '&charges=' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.discount_amount, charges.data.grand_total].join(',');
    return iframes.frame(url, false);
  }

  // charges is a charges object
  get_pdp_display(charges) {
    let view = 'pdp';
    const url = platform.pdpHost(platform.marketingUI, this.platform) + '/' + view + '.html?public_key=' + this.key + '&charges=' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.discount_amount, charges.data.grand_total].join(',');
    return iframes.frame(url);
  }
  get_cart_display(charges, desktop, mobile) {
    ...
    const url = platform.pdpHost(platform.marketingUI, this.platform) + '/' + view + '.html?public_key=' + this.key + '&desktop=' + desktop + '&mobile=' + mobile + '&charges=' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.discount_amount, charges.data.grand_total].join(',');
    return iframes.frame(url);
  }
