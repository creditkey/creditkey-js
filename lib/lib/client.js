...
var _modalPdpBanner = _interopRequireDefault(require("./components/modal-pdp-banner"));
...
var Client = /*#__PURE__*/function () {
  ...
  _proto.enhanced_pdp_modal = function enhanced_pdp_modal(charges, type) {
    ...
    var url = (0, _platform.pdpHost)(_platform.marketingUI, this.platform) + '/pdp/' + this.key + '/' + type + '/' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.grand_total].join(',');
    return (0, _modal["default"])(url);
  } //charges is a charges object
  ;

  _proto.get_pdp_display = function get_pdp_display(charges) {
    var url = (0, _platform.pdpHost)(_platform.marketingUI, this.platform) + '/pdp/' + this.key + '/' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.discount_amount, charges.data.grand_total].join(',');
    return (0, _modalPdpBanner["default"])(url);
  };

  _proto.get_cart_display_left = function get_cart_display_left(charges) {
    var url = (0, _platform.pdpHost)(_platform.marketingUI, this.platform) + '/cart-promo/left/' + this.key + '/' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.discount_amount, charges.data.grand_total].join(',');
    return (0, _modalPdpBanner["default"])(url);
  };

  _proto.get_cart_display_right = function get_cart_display_right(charges) {
    var url = (0, _platform.pdpHost)(_platform.marketingUI, this.platform) + '/cart-promo/right/' + this.key + '/' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.discount_amount, charges.data.grand_total].join(',');
    return (0, _modalPdpBanner["default"])(url);
  };

  _proto.get_cart_display_centered = function get_cart_display_centered(charges) {
    var url = (0, _platform.pdpHost)(_platform.marketingUI, this.platform) + '/cart-promo/centered/' + this.key + '/' + [charges.data.total, charges.data.shipping, charges.data.tax, charges.data.discount_amount, charges.data.grand_total].join(',');
    return (0, _modalPdpBanner["default"])(url);
  };
