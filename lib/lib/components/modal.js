...
function validate_url(url) {
  ...
  if (platform.api('development').split('/')[1] === root) return true;
  if (platform.api('staging').split('/')[1] === root) return true;
  if (platform.api('production').split('/')[1] === root) return true;
  return false;
}
